...
export async function GET(
...
): Promise<NextResponse<ApiResponse<Task[]> | { error: { code: string; message: string } }>> {
  ...
  // Return tasks owned by this agent (or all tasks for the main agent)
  const tasks = registry
    .getTasks()
    .filter((t) => t.owner === agent.id || (id === 'main' && !t.owner));
